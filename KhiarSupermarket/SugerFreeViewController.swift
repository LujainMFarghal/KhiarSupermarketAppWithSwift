...
class SugerFreeViewController: UIViewController, UICollectionViewDelegate , UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, productsCV , UISearchBarDelegate , productDetailsProtocol{

    @IBOutlet weak var productsSegments: UISegmentedControl!
    @IBOutlet weak var productsCollectionView: UICollectionView!
    var arrOfProducts = [productCell]()
    var ref = Database.database().reference()
    var sugerFreeAvoid = [String]()
    ...
    public var notForSugerfree = [String]()
    ...
    override func viewDidLoad() {
        ...
        if let path = Bundle.main.path(forResource: "sugerFreeAvoid", ofType: "txt") {
            do {
                let data = try String(contentsOfFile: path, encoding: .utf8)
                sugerFreeAvoid = data.components(separatedBy: ",")
            } catch {
                print(error)
            }
        }
        ref.child("Products").observe(.value , with: { (snapshot) in
            ...
            for products in snapshot.children.allObjects as! [DataSnapshot] {
                ...
                for notKeto in self.sugerFreeAvoid {
                    if productIngredients.contains(notKeto){
                        self.notForSugerfree.append(productName)
                    }
                }
                if !self.notForSugerfree.contains(productName) {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "SugerFreeProductsCell", for: indexPath) as! CustomerProductCollectionViewCell
        if search == false{
            if productSection == "All"{
