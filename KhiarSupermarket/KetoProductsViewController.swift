//
//  KetoProductsViewController.swift
//  KhiarSupermarket
//
//  Created by Lujain Farghal on 30/08/1444 AH.
//
...
class KetoProductsViewController: UIViewController ,  UICollectionViewDelegate , UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, productsCV, UISearchBarDelegate , productDetailsProtocol{
    ...
    override func viewDidLoad() {
        ...
        if let path = Bundle.main.path(forResource: "ketoAvoid", ofType: "txt") {
            do {
                let data = try String(contentsOfFile: path, encoding: .utf8)
                ketoAvoid = data.components(separatedBy: ",")
            } catch {
                print(error)
            }
        }
        ...
        ref.child("Products").observe(.value , with: { (snapshot) in
            ...
            for products in snapshot.children.allObjects as! [DataSnapshot] {
                ...
                for notKeto in self.ketoAvoid {
                    if productIngredients.contains(notKeto){
                        self.notForKeto.append(productName)
                    }
                }
                if !self.notForKeto.contains(productName) {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "KetoProductsCell", for: indexPath) as! CustomerProductCollectionViewCell
        if search == false{
            if productSection == "All"{
    ...
    @IBAction func productsChosenSegment(_ sender: Any) {
